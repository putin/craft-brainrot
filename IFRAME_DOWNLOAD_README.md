...
```bash
wget --recursive --no-parent --no-clobber --page-requisites --html-extension --convert-links --restrict-file-names=windows --domains craftbrainrot.net --no-check-certificate https://craftbrainrot.net/game/craft-brainrot/
```
...
- `--domains craftbrainrot.net`: 限制wget只下载来自craftbrainrot.net域名的文件，忽略其他域名的资源
...
gameSection: {
  game: {
    url: '/craft-brainrot.embed',
    externalUrl: '/game/craft-brainrot/index.html'
  }
}
...
async rewrites() {
  return [
    {
      source: '/craft-brainrot.embed',
      destination: '/game/craft-brainrot/index.html',
    },
  ]
}
```
